...
**`.text-gradient` (patent gradient on text) reads as flat purple unless the element shrink-wraps its content.** The class (`globals.css`) does `background-image: var(--gradient-signature); background-clip: text;` at a fixed 135deg across the *element's own box*, not across the glyphs. If that element is a block-level child of a wider parent (e.g. a grid column), its box stretches to the parent's width while the text only occupies the left portion of it — so you only ever see the first slice of the gradient (violet → early fuchsia), never reaching orange. This bit the About section KPI stats (`about.tsx`, the `15+` / `7+` / `10K+` / `2x+` numbers): they sat in a `grid-cols-4` `<dd>` and looked "too purple" until fixed. Fix: add `inline-block` (or `w-fit`) to the gradient-text element so its box sizes to the text itself, matching how the hero's `<span className="text-gradient">AI</span>` (`hero.tsx`) already behaves correctly by virtue of being inline. Check this any time `.text-gradient` is applied to a block-level element, especially inside a grid/flex layout.
