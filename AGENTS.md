...
Fix pattern (already applied in `navbar.tsx`, copy it for any future Sheet nav): make the `Sheet` controlled (`open`/`onOpenChange` state you own), intercept the link's `onClick` with `preventDefault()`, close the sheet, THEN scroll to the target. **Don't guess a fixed delay for "then"** — a hardcoded `setTimeout(..., 320)` was tried first and is itself a variant of this same bug: Radix's body lock (`document.body.style.pointerEvents = "none"`, paired with a `data-scroll-locked` attribute) doesn't clear on a fixed clock, so a fixed delay still races it under different conditions (slower devices, repeated open/close, etc.). Instead, watch for the actual release with a `MutationObserver` on `document.body`'s `style` attribute, and only scroll once `pointerEvents !== "none"` (keep a short timeout as a fallback in case the lock was never engaged). See the current implementation in `navbar.tsx`'s `handleLinkClick` before changing this again.
